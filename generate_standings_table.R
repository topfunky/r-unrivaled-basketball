...
for (season_year in seasons) {
  ...
  standings_file <- file.path(data_dir, "unrivaled_regular_season_standings.feather")
  elo_file <- file.path(data_dir, "unrivaled_final_elo_ratings.feather")
  
  # Skip if required files don't exist
  if (!file.exists(standings_file)) {
    message(sprintf("Standings file not found for season %d: %s\nSkipping season %d...", season_year, standings_file, season_year))
    next
  }
  if (!file.exists(elo_file)) {
    message(sprintf("ELO ratings file not found for season %d: %s\nSkipping season %d...", season_year, elo_file, season_year))
    next
  }
  ...
  # Save the data
  write_feather(final_stats, file.path(data_dir, "unrivaled_final_stats.feather"))
