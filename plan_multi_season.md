...
## 2. Scraper Logic Refactoring (`task02.R`)
...
## 3. Data Processing Pipeline (`task08.R`)
...
- **Filtering**: Update analysis scripts (`task01.R`, `render_stats.R`, etc.) to filter for a specific season or group by season when performing comparisons.
...
3. **Refactor**: Apply the parameterization changes to `task02.R` and `task08.R`.
