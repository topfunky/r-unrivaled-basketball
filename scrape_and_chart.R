...
scrape_game_data <- function(url) {
    ...
    for (game_div in game_divs) {
        ...
        game_data <- c(game_data, lapply(games, function(game) {
            team_a_score <- game %>% html_node("h3.weight-900") %>% html_text(trim = TRUE) %>% as.integer()
            team_a_name <- game %>% html_node(".color-blue.weight-500.font-14") %>% html_text(trim = TRUE)

            team_b <- game %>% html_nodes(".color-blue.weight-500.font-14") %>% .[2]
            team_b_name <- team_b %>% html_text(trim = TRUE)
            team_b_score <- team_b %>% html_node(xpath = "following-sibling::h3") %>% html_text(trim = TRUE) %>% as.integer()
...
ggplot(rankings, aes(x = Week, y = Rank, color = Team)) +
  ...
  labs(title = "Unrivaled Basketball League Rankings",
       x = "Week",
       y = "Rank",
       color = "Team") +
  theme_minimal()
