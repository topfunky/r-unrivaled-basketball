...
scrape_unrivaled_games <- function(season_year = 2025) {
  ...
  parse_game_day <- function(day_node) {
    ...
    if (season_year == 2026) {
      ...
      date_div <- day_node |>
        html_element("div.flex.justify-center.items-center.h-100.row-4.weight-500")
      if (is.null(date_div)) {
      ...
      date_text <- str_replace(date_text, "([A-Za-z]+)([0-9]+)([A-Za-z]+)", "\\1 \\2 \\3")
    } else {
    ...
    game_date <- if (str_detect(date_text, "^Today")) {
      ...
      if (is.na(d)) {
        ...
        date_parts <- str_match(date_text, "([A-Za-z]+)\\s*([0-9]+)\\s*([A-Za-z]+)")
    ...
    if (length(games_2025) >= 2) {
      ...
      print(paste("Found", length(games_2025), "team scores for this day (2025 format)"))
      games <- games_2025
      format_2026 <- FALSE
    } else if (length(games_2026) > 0) {
      # 2026 format: each link is a game
      print(paste("Found", length(games_2026), "games for this day (2026 format)"))
      games <- games_2026
      format_2026 <- TRUE
    } else {
  ...
  if (season_year == 2026 && (nrow(all_games) == 0 || nrow(all_games) < 20)) {
    ...
    tryCatch({
      ...
      if (is.null(final_indicator) || html_text(final_indicator) != "Final") {
        next
      }
      ...
      for (day_node in day_nodes) {
        ...
        day_game_ids <- map_chr(games_in_day, ~ {
        ...
        if (game_id %in% day_game_ids) {
          date_div <- html_element(day_node, "div.flex.justify-center.items-center.h-100.row-4.weight-500")
          if (!is.null(date_div)) {
            date_text <- html_text(date_div)
            date_text <- str_replace(date_text, "([A-Za-z]+)([0-9]+)([A-Za-z]+)", "\\1 \\2 \\3")
            date_parts <- str_match(date_text, "([A-Za-z]+)\\s*([0-9]+)\\s*([A-Za-z]+)")
...
for (season_year in seasons) {
  ...
  print(paste0("✅ Saved ", nrow(season_games), " games for season ", season_year, " to ", season_dir, "/unrivaled_scores.csv"))
}
